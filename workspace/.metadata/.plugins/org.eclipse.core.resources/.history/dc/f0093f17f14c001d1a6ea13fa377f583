...
public class NDFAutomaton {
    ...
    public static NDFAutomaton regExTree2NFA(RegExTree ret) {
        ...
        if (ret.root == RegEx.CONCAT) {
            ...
            for (int i = 0; i < len_g; i++) for (int col = 0; col < 256; col++) tTab[i][col] = tTab_g[i][col];
            for (int i = 0; i < len_g; i++) eTab[i].addAll(eTab_g[i]);
            for (int i = len_g; i < len_g + len_d - 1; i++)
                for (int col = 0; col < 256; col++)
                    if (tTab_d[i - len_g][col] != -1) tTab[i][col] = tTab_d[i - len_g][col] + len_g;
            for (int i = len_g; i < len_g + len_d - 1; i++)
                for (int s : eTab_d[i - len_g]) eTab[i].add(s + len_g);
            return new NDFAutomaton(tTab, eTab);
